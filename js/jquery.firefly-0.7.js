...
(function ($) {
  ...
  $.firefly = function (settings) {
    ...
    for (i = 0; i < $.firefly.settings.total; i++) {
      // Run the next few lines of code for every firefly
      var randomPixel = $.firefly.randomPixel(
        $.firefly.settings.minPixel,
        $.firefly.settings.maxPixel
      ); // Get the size of the specific firefly
    ...
    $.firefly.sparks = $($.firefly.settings.on).children(
      "." + $.firefly.settings.namespace
    ); // Create a list of all of the fireflies
